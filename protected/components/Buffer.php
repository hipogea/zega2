...
class Buffer extends CDbCriteria
{
    ...
    protected function ConfigArreglos() {
		...
		///Los modelos que no aprecen en la matriz links son los modelos nietos
		$modnietos=array_diff(array_keys($this->modeloshijos),array_keys($this->camposlink));
		foreach($modnietos as $clav=>$val){
			$matriz=$val::relations();
			foreach($matriz as $clave=>$matricita)
			{
				if($matricita[0]==$val::HAS_MANY)
					$this->camposlink[$matricita[1]]=$matricita[2];
			}
		}
		...
		foreach($this->modeloshijos as $nametablaoriginal => $nametablatemporal)
		  {
			 ...
			 // var_dump($nametablaoriginal);
			  if(in_array($nametablaoriginal,array_keys($this->relacionesnietas())))
			  {
				  $hj=$this->relacionesnietas()[$nametablaoriginal];
				  $id=$hj::model()->findByPk();
			  }
