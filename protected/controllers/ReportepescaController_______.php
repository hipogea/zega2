...
return array(
	...
	public function accessRules()
	{
		return array(
			...
			array('allow', // allow authenticated user to perform 'create' and 'update' actions
				'actions'=>array('eficiencia','gestionaparte','gestionaparte1'),
				'users'=>array('@'),
			),
	...
	public function actionCrearparte($idtemporada)
	{
	 ...
	 // $this->performAjaxValidation($model);
		...
		if(isset($_POST['Temporadas']))
		{
			...
			$command = Yii::app()->db->createCommand("select fn_crea_reporte_pesca('".$model->fechadehoy."',".$model->idespecie.",".$model->id.")"); 
			$command->execute();
			$this->redirect(array('gestionaparte','fecha'=>$model->fechadehoy));
			Yii::app()->end();
			//}
		}
	...
	public function actionGestionaparte($fecha)
	{
		...
		$criteriazo->with = array('embarcacion');
		 if($modeloreportes->embarcacion_nomep){
				$criteriazo->compare('embarcacion.nomep',$modeloreportes->embarcacion_nomep,true);
			}
			...
			if(isset($_GET['Reportepesca']))
				$modeloreportes->attributes=$_GET['Reportepesca'];	
				           // $this->layout = '//layouts/column3';
							...
							$this->render('reportepordia_',array('idtemporada'=>$vtemporada,'idespecie'=>$vespecie,'fecha'=>$fecha,'proveedorreportes'=>$proveedorreportes,'modeloreportes'=>$modeloreportes));
	...
	public function actionGestionaparte1($fecha)
	{
			...
			$modeloreportes=	new VwPartediarioPesca;
							$modeloreportes->unsetAttributes(); 
						$criteriazo=new CDbCriteria;
						$criteriazo->addCondition('fecha = :idfecha');
						$criteriazo->params = array(':idfecha' => $fecha);
			...
			if(isset($_GET['VwPartediarioPesca']))
				$modeloreportes->attributes=$_GET['VwPartediarioPesca'];	
				           // $this->layout = '//layouts/column3';
							 $this->layout = '';
...
//$model=new Reportepesca('search');
						...
						$model->setScenario('miescenario');
						  if(($model->criticidad =='A') OR ($model->criticidad =='B')) 
								//$this->enviacorreo(substr($model->descridetalle,0,35),$model->descridetalle,$model->idnovedad);
						         $this->enviacorreo(substr($modelito,$model));
	...
	public function actionActualizanovedad($novel)
	{
	...
	public function actionRespondenovedad($novel)
	{
		 $model=$this->cargaNovedad($novel);
		 $modelito=$this->loadModel($novel);
		  if (!is_null($model))  {
	...
	public function enviacorreo($modelito,$modelo)
			{
				$lista=array(
				...
				//'jpanta@exalmar.com.pe',
				//'ovalenzuela@exalmar.com.pe'
				);
				$titulo="NOVEDAD-";
				//agregando el usuario que genera este correo
				array_push($lista,Yii::app()->user->email);				
				$titulo.=substr($modelo->descridetalle,0,35);
				$contenido=$modelo->descridetalle;
				$contenido.="<br>";
				$contenido.=$this->widget('zii.widgets.CDetailView', array(
	'data'=>$model,
	'attributes'=>array(
		'codep',
		'id',
		...
		'codzarpe',
				...
				Yii::app()->crugemailer->mail_novedadradio($lista,$titulo,$contenido);	         
				
			
			
			/*$modeloreporte=
			$modeloparte=Partes::model()->find('id=:arte',array(':arte'=>$id));
