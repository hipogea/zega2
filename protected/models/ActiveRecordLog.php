...
class ActiveRecordLog extends CActiveRecord
{
	...
	public static function camposclave(){
		return array('campo1','campo2','campo3');
	}

	public static function hacercriterio($nombremodelo,$vaL){
		$criter=New CDBCriteria();
		//$vaL=$this->Owner->getPrimaryKey();
		IF(IS_ARRAY($vaL))
		{
			$criter->addcondition(" model=:vmodelo ");
			if(count($vaL)<=count(self::camposclave())){
				$i=0;
				foreach($vaL as $clave=>$valor){
					$criter->addcondition(self::camposclave()[$i]."='".$valor."'","AND");
					$i+=1;
				}
			}else{
				//errror no yah mas campos
			}

		}ELSE{

			$criter->addcondition(" model=:vmodelo and idModel=:vidclave");
			$criter->params=array(":vmodelo"=>$nombremodelo,":vidclave"=>$vaL);
		}
		RETURN $criter;
	}

	public function asignacamposclave($valores){
		IF(IS_ARRAY($valores)){
			$i=0;
			foreach($valores as $clave=>$valor){
				$this->{$this->camposclave()[$i]}=$valor;
				$i+=1;
			}
		}
		return true;
